...
1. Copy the prepared `deploy/` directory to `/opt/meet37`. It includes `.env.example`; the server `.env` is generated by `prepare_server.py`.
2. Put TLS files in `/opt/meet37/caddy/certs/`.
...
4. Load the offline image bundle:
