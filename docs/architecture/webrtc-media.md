...
meet37 uses WebRTC for real-time camera, microphone, screen sharing, and data channels. The backend does not proxy normal HTTP media. It coordinates signaling and can provide SFU/media coordination through Pion WebRTC.
...
8. The backend may trigger `sfu-switch` when stats indicate poor connectivity.
...
The frontend also calculates audio level locally and broadcasts `audio-level` over signaling. Remote clients use this to show speaking indicators. This signal is a UI indicator only; it is not a replacement for the actual audio track.
...
- Docker must publish `TURN_HOST_PORT` for TCP and UDP.
